import { GitHubReadmeRequestSchema } from "@/lib/github/contracts";
import {
  fetchGitHubReadme,
  GitHubClientError,
...
export async function POST(request: Request) {
  ...
  const parsed = GitHubReadmeRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const { readme } = await fetchGitHubReadme(
      ...
      parsed.data.repo,
      { maxBytes: limits.maxReadmeBytes },
    );
    return Response.json({ readme }, { headers: NO_STORE_HEADERS });
  } catch (error) {
    if (error instanceof GitHubClientError) {
      ...
      if (error.kind === "not_found") {
        return failure("Repository or README not found.", 404);
      }
      return failure("GitHub is temporarily unavailable. Try again.", 503);
    }
    return failure("Could not fetch README.", 500);
  }
}
